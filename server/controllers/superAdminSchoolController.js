    ...
    try {
        ...
        if (error.code === "23505") {
            return res.status(409).json({
                success: false,
                message: "The administrator username/email or another unique school value already exists."
            });
        }
        if (error.status) {
...
const setSchoolStatus = async (req, res, next) => {
    try {
        ...
        res.json({
            success: true,
            message: `School ${school.is_active ? "activated" : "deactivated"} successfully.`,
            data: school
        });
    } catch (error) {
        next(error);
