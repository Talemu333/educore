...
const getAvailableStudentExams = async (studentId, schoolId) => {
    const result = await pool.query(`
        SELECT e.id,e.title,e.description,e.duration_minutes,e.total_marks,e.pass_mark,e.max_attempts,
               e.randomize_questions,e.randomize_options,e.show_result_immediately,e.starts_at,e.ends_at,
...
const createExam = async (data, schoolId, userId) => {
    ...
    const result=await pool.query(`INSERT INTO cbt_exams (school_id,subject_id,class_id,arm_id,title,description,duration_minutes,total_marks,pass_mark,max_attempts,randomize_questions,randomize_options,show_result_immediately,starts_at,ends_at,status,created_by) VALUES ($1,$2,$3,$4,$5,$6,$7,0,$8,$9,$10,$11,$12,$13,$14,$15,$16) RETURNING *`,[schoolId,data.subject_id,data.class_id,data.arm_id||null,data.title,data.description||null,data.duration_minutes,data.pass_mark||0,data.max_attempts||1,Boolean(data.randomize_questions),Boolean(data.randomize_options),data.show_result_immediately!==false,data.starts_at||null,data.ends_at||null,data.status||"draft",userId]);
    return result.rows[0];
};
...
const updateExam = async (examId,data,schoolId) => {
    ...
    try{
        ...
        const result=await client.query(`UPDATE cbt_exams SET subject_id=$3,class_id=$4,arm_id=$5,title=$6,description=$7,duration_minutes=$8,total_marks=$9,pass_mark=$10,max_attempts=$11,randomize_questions=$12,randomize_options=$13,show_result_immediately=$14,starts_at=$15,ends_at=$16,status=$17,updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND school_id=$2 RETURNING *`,[examId,schoolId,data.subject_id,data.class_id,data.arm_id||null,data.title,data.description||null,data.duration_minutes,totals.totalMarks,data.pass_mark||0,data.max_attempts||1,Boolean(data.randomize_questions),Boolean(data.randomize_options),data.show_result_immediately!==false,data.starts_at||null,data.ends_at||null,data.status||"draft"]);
...
const saveAnswer=async(attemptId,questionId,selectedOptionId,schoolId)=>{
    const result=await pool.query(`
        ...
        FROM cbt_attempts a
        JOIN cbt_questions q ON q.id=$2 AND q.exam_id=a.exam_id AND q.school_id=$4
