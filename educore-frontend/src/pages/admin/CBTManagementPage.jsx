...
function CBTManagementPage() {
    ...
    const openQuestions = async (item) => {
        try {
            ...
            const loadedExam = response.data?.data || null;
            setSelectedExam(loadedExam);
            setExam((current) => ({ ...current, total_marks: Number(loadedExam?.total_marks || 0) }));
            setQuestion({ ...emptyQuestion, question_order: (loadedExam?.questions?.length || 0) + 1 });
    ...
    const saveQuestion = async (event) => {
        ...
        try {
            ...
            toast.success("Question added. Total marks updated automatically.");
            ...
            <Card>
                <CardContent className="p-5 sm:p-7">
                    ...
                    <form onSubmit={saveExam} className="grid gap-4 md:grid-cols-2">
                        ...
                        <label className="rounded-lg border p-3 bg-slate-50">Total marks<input className="mt-1 w-full cursor-not-allowed bg-transparent outline-none" type="number" min="0" value={exam.total_marks} readOnly /><span className="mt-1 block text-xs text-slate-500">Calculated automatically from the marks assigned to questions.</span></label>
                        <label className="rounded-lg border p-3">Pass mark (%)<input className="mt-1 w-full outline-none" type="number" min="0" max="100" value={exam.pass_mark} onChange={(e) => setExam({ ...exam, pass_mark: Number(e.target.value) })} /></label>
