...
function ReportsPage() {
    ...
    useEffect(() => {
        const loadPublication = async () => {
            if (!sessionId || !termId || !classId || !armId) {
                setPublication(null);
                return;
            }
            ...
            try {
                const response = await api.get("/results/publication", {
                    params: { sessionId, termId, classId, armId },
                });
        ...
        loadPublication();
    }, [sessionId, termId, classId, armId]);
    ...
    const publishResults = async () => {
        ...
        if (!window.confirm("Publish this result for students? Students in this class and arm will be able to view their report for the selected term.")) {
            return;
        }
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title="Reports"
                description="Review academic reports and control when term results become visible to students and parents."
            />
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                Publish a class result after checking the report sheet. Students and parents can only view results after publication.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-5 sm:p-6">
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <label className="text-sm font-medium text-slate-700">
                            ...
                            >
                                ...
                                {arms.map(arm => (
                                    <option key={arm.id} value={arm.id}>{arm.arm_name}</option>
                                ))}
                            </select>
                        </label>
                    </div>
                    ...
                    <div className="mt-6 flex flex-col gap-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="font-semibold text-slate-900">
                                    {publication ? "Result published" : "Result not published"}
                                </p>
                                <p className="mt-1 text-sm text-slate-500">
                                    {publication
                        ...
                        <Button
                            ...
                            disabled={publishing || !sessionId || !termId || !classId || !armId || Boolean(publication)}
                        >
