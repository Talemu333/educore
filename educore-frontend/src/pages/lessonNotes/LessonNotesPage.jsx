...
function LessonNotesPage() {
    ...
    const selectTopic = topic => {
        setForm(current => ({ ...current, topic: topic.topic, sub_topic: topic.sub_topic || "" }));
    };
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title="Lesson Notes"
                description={isTeacher ? "Prepare, submit and track your lesson notes." : "View, review and manage lesson notes submitted by teachers."}
            ...
            {formOpen && <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/50 p-4"><div className="mx-auto my-6 max-w-5xl rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-6 py-4"><div><h2 className="text-lg font-bold text-slate-900">{editing ? "Edit Lesson Note" : "New Lesson Note"}</h2><p className="text-sm text-slate-500">Saved first as a draft.</p></div><button onClick={() => setFormOpen(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><form onSubmit={submitForm} className="space-y-5 p-6">
                ...
                {isTeacher && <div><label className="mb-1 block text-sm font-medium text-slate-700">Teaching Assignment</label><select required value={`${form.class_id}|${form.subject_id}|${form.session_id}|${form.term_id}`} onChange={e => { const assignment = assignments.find(item => `${item.class_id}|${item.subject_id}|${item.session_id}|${item.term_id}` === e.target.value); if (assignment) selectAssignment(assignment); }} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm"><option value="">Select class / subject</option>{assignments.map((assignment, index) => <option key={`${assignment.class_id}-${assignment.subject_id}-${assignment.session_id}-${assignment.term_id}-${index}`} value={`${assignment.class_id}|${assignment.subject_id}|${assignment.session_id}|${assignment.term_id}-${index}`}>{assignment.class_name} — {assignment.subject_name} — {assignment.session_name} / {assignment.term_name}</option>)}</select></div>}
                <div className="grid gap-4 md:grid-cols-4"><div><label className="mb-1 block text-sm font-medium text-slate-700">Week</label><input required type="number" min="1" max="52" value={form.week_number} onChange={e => setForm({ ...form, week_number: e.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" /></div><div><label className="mb-1 block text-sm font-medium text-slate-700">Lesson Date</label><input type="date" value={form.lesson_date} onChange={e => setForm({ ...form, lesson_date: e.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" /></div><div><label className="mb-1 block text-sm font-medium text-slate-700">Duration</label><input value={form.duration} onChange={e => setForm({ ...form, duration: e.target.value })} placeholder="e.g. 40 minutes" className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" /></div><div><label className="mb-1 block text-sm font-medium text-slate-700">Topic</label><select required value={form.topic} disabled={isTeacher && topicsLoading} onChange={e => { const topic = topics.find(item => item.topic === e.target.value); if (topic) selectTopic(topic); else setForm({ ...form, topic: e.target.value }); }} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm"><option value="">{topicsLoading ? "Loading syllabus topics..." : topics.length ? "Select syllabus topic" : "Select assignment first"}</option>{topics.map(topic => <option key={topic.id} value={topic.topic}>{topic.topic}{topic.sub_topic ? ` — ${topic.sub_topic}` : ""}</option>)}</select></div></div>
                {!isTeacher && <p className="text-xs text-slate-500">Administrators may prepare a note for a teacher. Teachers must select a topic from the approved syllabus for the selected week.</p>}
