        ...
        return (
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    {[1, 2, 3, 4, 5].map(item => (
                        <div key={item} className="h-24 animate-pulse rounded-2xl bg-slate-200 sm:h-28" />
                    ))}
                </div>
            </div>
        );
        ...
        <div className="w-full min-w-0 space-y-5 sm:space-y-6">
            ...
            <section>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto]">
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        ...
                        <p className="mt-1.5 break-words text-xl font-bold text-slate-900 sm:text-2xl">{formatCurrency(expected_fees)}</p>
                    </div>
                    <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
                        <p className="text-sm font-medium text-emerald-700">Total Payments</p>
                        <p className="mt-1.5 break-words text-xl font-bold text-emerald-800 sm:text-2xl">{formatCurrency(total_payments)}</p>
                    </div>
                    <div className="rounded-2xl border border-red-100 bg-red-50 p-4">
                        <p className="text-sm font-medium text-red-700">Outstanding Fees</p>
                        <p className="mt-1.5 break-words text-xl font-bold text-red-800 sm:text-2xl">{formatCurrency(outstanding_fees)}</p>
                    </div>
                    <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
                        <p className="text-sm font-medium text-blue-700">Students With Payments</p>
                        <p className="mt-1.5 text-xl font-bold text-blue-800 sm:text-2xl">{students_with_payments}</p>
                    </div>
                </div>
            </section>
                        ...
                        {gender_distribution.map(item => {
                            ...
                            return (
                                <div key={item.gender}>
                                    ...
                                    <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600" style={{ width: `${percentage}%` }} /></div>
                                </div>
                            );
                        })}
                    </div>
                ...
                <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                    ...
                    <div className="mt-4 space-y-2.5">
                        {class_population.map(item => (
                            <div key={item.class_name} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2.5 sm:px-4">
                                <span className="min-w-0 truncate font-medium text-slate-700">{item.class_name}</span>
                                <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-sm font-bold text-slate-700 shadow-sm">{item.total_students}</span>
                            </div>
                        ))}
                    </div>
                </section>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-4 py-4 sm:px-5"><h2 className="text-lg font-bold text-slate-900">Recent Students</h2><p className="mt-0.5 text-sm text-slate-500">Recently registered students.</p></div>
                    <div className="divide-y divide-slate-100">
                        {recent_students.map(student => (
                            <div key={student.id} className="flex flex-col gap-0.5 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                                <div className="min-w-0"><p className="truncate font-semibold text-slate-800">{student.surname} {student.first_name} {student.middle_name || ""}</p><p className="mt-0.5 text-xs text-slate-400">{student.admission_number}</p></div>
                ...
                <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-4 py-4 sm:px-5"><h2 className="text-lg font-bold text-slate-900">Top Students</h2><p className="mt-0.5 text-sm text-slate-500">Highest average scores.</p></div>
                    <div className="divide-y divide-slate-100">
                        {top_students.map((student, index) => (
                            <div key={student.id} className="flex items-center gap-3 px-4 py-3 sm:gap-4 sm:px-5">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-4 py-4 sm:px-5"><h2 className="text-lg font-bold text-slate-900">Recent Payments</h2><p className="mt-0.5 text-sm text-slate-500">Latest fee transactions.</p></div>
                    <div className="divide-y divide-slate-100">
                        {recent_payments.map(payment => (
                            <div key={payment.id} className="px-4 py-3 sm:px-5">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0"><p className="truncate font-semibold text-slate-800">{payment.student_name}</p><p className="mt-0.5 break-words text-xs text-slate-400">{payment.reference_number} • {payment.payment_method}</p></div>
                                    <p className="shrink-0 text-right font-bold text-emerald-600">{formatCurrency(payment.amount_paid)}</p>
                                </div>
                                <p className="mt-1.5 text-xs text-slate-400">{formatDate(payment.payment_date)}</p>
                            </div>
                        ))}
                    </div>
                </section>
                ...
                <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-4 py-4 sm:px-5"><h2 className="text-lg font-bold text-slate-900">Recent Announcements</h2><p className="mt-0.5 text-sm text-slate-500">Latest school announcements.</p></div>
                    <div className="divide-y divide-slate-100">
                        {recent_announcements.map(announcement => (
                            <div key={announcement.id} className="px-4 py-3 sm:px-5">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0"><p className="font-semibold text-slate-800">{announcement.title}</p><p className="mt-0.5 line-clamp-2 text-sm text-slate-500">{announcement.message}</p></div>
                                ...
                                <p className="mt-1.5 text-xs text-slate-400">Published {formatDate(announcement.publish_date)}</p>
                            </div>
                        ))}
                    </div>
                </section>
