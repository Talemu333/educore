...
export async function syncPendingAttendance() {
    ...
    if (!navigator.onLine) return { synced: 0, remaining: await getPendingSyncItems() };
    ...
    syncPromise = (async () => {
        ...
        for (const item of items) {
            if (Number(item.schoolId) !== Number(user.school_id)) {
                continue;
            }
            ...
            try {
                ...
                if (isNetworkFailure(error)) {
                    await markSyncItemPending(
                        item,
                        error?.userMessage || "Network unavailable"
                    );
                    break;
                }

                // Keep server validation/authentication failures in the queue
                // for inspection/retry rather than silently losing the record.
                await markSyncItemPending(
                    item,
